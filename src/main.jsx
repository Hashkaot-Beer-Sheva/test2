...
function App() {
  ...
  useEffect(() => { if (!authenticated || cloudDataStatus !== 'Cloud spreadsheets loaded' || occupancyLoaded.current) return undefined; occupancyLoaded.current = true; let cancelled = false; fetch('/api/data').then((response) => response.ok ? response.json() : null).then((payload) => { if (cancelled) return; const parsed = parseOccupancyCsv(payload?.files?.occupancy || ''); if (!parsed.records.length) return; setData((current) => { const apartments = current.apartments.map((apartment) => { const apartmentUnits = [apartment.renterUnit, apartment.renterCsv?.unit, apartment.unifiedOccupancyUnit].filter(Boolean).flatMap((value) => String(value).split('/')).map((value) => unitKey(value.trim())).filter(Boolean); const matchingRecords = parsed.records.filter((record) => apartmentUnits.includes(unitKey(record.unit)));  const latestUnit = apartmentUnits.find((unit) => Object.prototype.hasOwnProperty.call(parsed.latest, unit)); return matchingRecords.length ? { ...apartment, rent: latestUnit ? parsed.latest[latestUnit] : apartment.rent, occupancyRentHistory: matchingRecords } : apartment; }); const transactions = (current.transactions || []).filter((item) => !String(item.id || '').startsWith('occupancy-rent-')); parsed.records.forEach((record, index) => { const apartment = apartments.find((candidate) => [candidate.renterUnit, candidate.renterCsv?.unit, candidate.unifiedOccupancyUnit, candidate.uniqueId, candidate.apptId, candidate.number].filter(Boolean).flatMap((value) => String(value).split('/')).map((value) => unitKey(value.trim())).includes(unitKey(record.unit))); if (!apartment) return; const id = `occupancy-rent-${unitKey(record.unit)}-${record.date}-${index}`; if (!transactions.some((item) => item.id === id)) transactions.push({ id, apartmentId: apartment.id, Unit: record.unit, Description: transactionDate(record.date) <= new Date() ? 'Rent received' : 'Rent expected', Amount: record.amount, 'Order Date': record.date, Note: record.status }); }); return { ...current, apartments, transactions }; }); }).catch(() => {}); return () => { cancelled = true; }; }, [authenticated, cloudDataStatus]);
