...
function ApartmentPhotoGallery({ apartment }) {
  ...
  useEffect(() => { let cancelled = false; if (!units.length) { setRemotePhotos([]); return undefined; } Promise.all(units.map((unit) => fetch(`/api/photos?unit=${encodeURIComponent(unit)}`).then((response) => response.ok ? response.json() : { photos: [] }).catch(() => ({ photos: [] })))).then((results) => { if (!cancelled) { setRemotePhotos([...new Map(results.flatMap((result) => result.photos || []).map((photo) => [photo.key, photo])).values()]); setIndex(0); } }); return () => { cancelled = true; }; }, [apartment.id, units.join('|')]);
