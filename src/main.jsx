...
const withoutPhotoBlobs = (value) => ({ ...value, apartments: (value.apartments || []).map(({ photoUrls, ...apartment }) => apartment) });
const googleStreetViewUrl = (building) => { const lat = Number(building?.lat); const lng = Number(building?.lng); return Number.isFinite(lat) && Number.isFinite(lng) ? `https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${lat},${lng}` : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(building?.address || building?.name || '')}`; };
...
const hydrateData = (loadMock = false) => {
  ...
  } : { buildings: [], apartments: [], people: [], bills: [], events: [], maintenance: [], transactions: [] };
  const apartments = [
    ...(base.apartments || []),
    ...(loadMock ? mockExpansion.apartments.filter(
      (a) => !(base.apartments || []).some((item) => item.id === a.id)
    ) : []),
    ...(loadMock ? bulkApartments.filter((a) => !(base.apartments || []).some((item) => item.id === a.id)) : []),
    ...(loadMock ? spreadsheetImport.apartments.filter(
      (a) => !(base.apartments || []).some((item) => item.id === a.id)
    ) : []),
  ];
  ...
  return {
    ...
    buildings: [
      ...(base.buildings || []),
      ...(loadMock ? mockExpansion.buildings.filter(
        (b) => !(base.buildings || []).some((item) => item.id === b.id)
      ) : []),
      ...(loadMock ? spreadsheetImport.buildings.filter(
        (b) => !(base.buildings || []).some((item) => item.id === b.id)
      ) : []),
    ],
    ...
    people: [
      ...(base.people || []),
      ...(loadMock ? mockExpansion.people.filter((p) => !(base.people || []).some((item) => item.id === p.id)) : []),
      ...(loadMock ? spreadsheetImport.people.filter(
        (p) => !(base.people || []).some((item) => item.id === p.id)
      ) : []),
    ],
    events: [
      ...(base.events || []),
      ...(loadMock ? bulkEvents.filter((event) => !(base.events || []).some((item) => item.id === event.id)) : []),
    ],
    bills: [
      ...(base.bills || []),
      ...(loadMock ? bulkBills.filter((bill) => !(base.bills || []).some((item) => item.id === bill.id)) : []),
    ],
    maintenance: [
      ...(base.maintenance || []),
      ...(loadMock ? operations.maintenance.filter(
...
const parseUnifiedCsv = (text) => { const delimiter = String(text || '').split(/\r?\n/).find((line) => line.includes('\t')) ? '\t' : ','; const records = parseDelimitedRecords(String(text || ''), delimiter); if (!records.length) return []; const headers = records[0].map((value) => String(value || '').replace(/^\uFEFF/, '').replace(/\s+/g, ' ').trim()); return records.slice(1).map((values) => Object.fromEntries(headers.map((header, index) => [header, String(values[index] || '').trim()]))).filter((row) => Object.values(row).some(Boolean)); };
const createUnifiedFirstData = (text, base = {}) => { const rows = parseUnifiedCsv(text); const buildings = []; const apartments = []; const buildingMap = new Map(); rows.forEach((row, index) => { const unifiedId = String(row[Object.keys(row)[0]] || '').trim(); if (!unifiedId || apartments.some((item) => unitKey(item.unifiedId) === unitKey(unifiedId))) return; const address = String(row['NEW OCCUPANCY ADRESSS'] || row['zehavit address'] || '').trim(); const match = address.match(/^(.+?)\s+(\d+[A-Za-zא-ת\/-]*)$/); const street = match?.[1] || address; const streetNumber = match?.[2] || ''; const buildingKey = `${normalizeCoordinateText(street)}|${unitKey(streetNumber)}`; let building = buildingMap.get(buildingKey); if (!building) { building = { id: `unified-building-${buildings.length + 1}`, name: `${street} ${streetNumber}`.trim() || `Unified building ${index + 1}`, street, streetNumber, address, area: 'Shechuna Gimel', units: 0, floors: 0, zone: 'Residential', color: 'gold', lat: null, lng: null }; buildingMap.set(buildingKey, building); buildings.push(building); } building.units += 1; apartments.push({ id: `unified-apartment-${unifiedId}`, unifiedId, buildingId: building.id, uniqueId: String(row['EASYPROD - UniquID'] || '').trim(), apptId: String(row['EASYPROD - Appt ID'] || '').trim(), number: '', entrance: '', street, streetNumber, addressNotEditable: address, altAddress: String(row['zehavit address'] || '').trim(), unifiedOccupancyUnit: String(row['NEW OCCUPANCY UNIT'] || '').trim(), unifiedOccupancyAddress: String(row['NEW OCCUPANCY ADRESSS'] || '').trim(), unifiedZehavitUnit: String(row['ZEHAVIT yechida'] || '').trim(), unifiedZehavitAddress: String(row['zehavit address'] || '').trim(), status: 'Vacant', rent: 0, tenantName: '', resident: '' }); }); return { ...base, buildings, apartments }; };
const enrichUnifiedWithMaster = (current, text) => { const imported = parseMasterCsv(text); const byKey = new Map((imported.apartments || []).map((item) => [`${unitKey(item.uniqueId)}|${unitKey(item.apptId)}`, item])); const byUnique = new Map((imported.apartments || []).map((item) => [unitKey(item.uniqueId), item])); const buildings = [...(current.buildings || [])]; const buildingByAddress = new Map(buildings.map((item) => [`${normalizeCoordinateText(item.street)}|${unitKey(item.streetNumber)}`, item])); const apartments = (current.apartments || []).map((apartment) => { const source = byKey.get(`${unitKey(apartment.uniqueId)}|${unitKey(apartment.apptId)}`) || byUnique.get(unitKey(apartment.uniqueId)); if (!source) return apartment; let building = buildingByAddress.get(`${normalizeCoordinateText(source.street)}|${unitKey(source.streetNumber)}`); if (!building) { building = { id: `easyprod-building-${buildings.length + 1}`, name: `${source.street} ${source.streetNumber}`.trim(), street: source.street, streetNumber: source.streetNumber, address: source.addressNotEditable || `${source.street} ${source.streetNumber}`, area: 'Shechuna Gimel', units: 0, floors: 0, zone: 'Residential', color: 'gold', lat: null, lng: null }; buildings.push(building); buildingByAddress.set(`${normalizeCoordinateText(source.street)}|${unitKey(source.streetNumber)}`, building); } if (apartment.buildingId !== building.id) building.units = Number(building.units || 0) + 1; return { ...apartment, ...source, id: apartment.id, unifiedId: apartment.unifiedId, buildingId: building.id, unifiedOccupancyUnit: apartment.unifiedOccupancyUnit, unifiedOccupancyAddress: apartment.unifiedOccupancyAddress, unifiedZehavitUnit: apartment.unifiedZehavitUnit, unifiedZehavitAddress: apartment.unifiedZehavitAddress }; }); return { ...current, buildings, apartments }; };
const applyUnifiedCrosswalk = (apartments, text) => { const rows = parseUnifiedCsv(text); return apartments.map((apartment) => { const row = rows.find((item) => unitKey(item['EASYPROD - UniquID']) === unitKey(apartment.uniqueId) && unitKey(item['EASYPROD - Appt ID']) === unitKey(apartment.apptId)) || rows.find((item) => unitKey(item['EASYPROD - UniquID']) === unitKey(apartment.uniqueId)) || rows.find((item) => unitKey(item['EASYPROD - Appt ID']) === unitKey(apartment.apptId)); return row ? { ...apartment, unifiedId: row[Object.keys(row)[0]], unifiedOccupancyUnit: row['NEW OCCUPANCY UNIT'], unifiedOccupancyAddress: row['NEW OCCUPANCY ADRESSS'], unifiedZehavitUnit: row['ZEHAVIT yechida'], unifiedZehavitAddress: row['zehavit address'] } : apartment; }); };
const buildUnifiedFirstData = (master, text) => { const rows = parseUnifiedCsv(text); const apartments = applyUnifiedCrosswalk(master.apartments || [], text); const buildings = [...(master.buildings || [])]; const byUnified = new Map(apartments.filter((item) => item.unifiedId).map((item) => [unitKey(item.unifiedId), item])); rows.forEach((row, index) => { const unifiedId = String(row[Object.keys(row)[0]] || '').trim(); if (!unifiedId || byUnified.has(unitKey(unifiedId))) return; const address = String(row['NEW OCCUPANCY ADRESSS'] || row['zehavit address'] || '').trim(); const match = address.match(/^(.+?)\s+(\d+[A-Za-zא-ת\/-]*)$/); const street = match?.[1] || address; const streetNumber = match?.[2] || ''; let building = buildings.find((item) => normalizeCoordinateText(item.street) === normalizeCoordinateText(street) && unitKey(item.streetNumber) === unitKey(streetNumber)); if (!building) { building = { id: `unified-building-${buildings.length + 1}`, name: `${street} ${streetNumber}`.trim() || `Unified building ${index + 1}`, street, streetNumber, address, area: 'Shechuna Gimel', units: 0, floors: 0, zone: 'Residential', color: 'gold', lat: null, lng: null }; buildings.push(building); } building.units += 1; const apartment = { id: `unified-apartment-${unifiedId}`, unifiedId, buildingId: building.id, uniqueId: row['EASYPROD - UniquID'] || '', apptId: row['EASYPROD - Appt ID'] || '', number: '', entrance: '', street, streetNumber, addressNotEditable: address, altAddress: row['zehavit address'] || '', unifiedOccupancyUnit: row['NEW OCCUPANCY UNIT'] || '', unifiedOccupancyAddress: row['NEW OCCUPANCY ADRESSS'] || '', unifiedZehavitUnit: row['ZEHAVIT yechida'] || '', unifiedZehavitAddress: row['zehavit address'] || '', status: 'Vacant', rent: 0, tenantName: '', resident: '' }; apartments.push(apartment); byUnified.set(unitKey(unifiedId), apartment); }); return { ...master, buildings, apartments }; };
const occupancyMatchesApartment = (record, apartment, unifiedRows) => { const crosswalk = unifiedRows.find((row) => unitKey(row['NEW OCCUPANCY UNIT']) === unitKey(record.unit)); if (crosswalk) return (unitKey(crosswalk['EASYPROD - UniquID']) === unitKey(apartment.uniqueId) && (!crosswalk['EASYPROD - Appt ID'] || unitKey(crosswalk['EASYPROD - Appt ID']) === unitKey(apartment.apptId))) || unitKey(crosswalk['NEW OCCUPANCY UNIT']) === unitKey(apartment.unifiedOccupancyUnit); return unitKey(apartment.unifiedOccupancyUnit) === unitKey(record.unit); };
...
const parseOccupancyCsv = (text) => { const lines = String(text || '').split(/\r?\n/); const delimiter = lines.find((line) => line.includes('\t')) ? '\t' : ','; const rows = parseDelimitedRecords(String(text || ''), delimiter, true).map((row) => row.map((value) => String(value || '').replace(/^\uFEFF/, '').trim())); const headerRow = rows[7] || []; const addressRow = rows[8] || []; const monthStart = 110; const columns = headerRow.map((unit, index) => ({ unit: String(unit || '').trim(), address: String(addressRow[index] || '').trim(), index })).filter((item) => item.unit && !/^(unit|יחידה)$/i.test(item.unit)); const latest = {}; const records = []; const zeroStatuses = /^(READY|Awaiting Payment|unpaid)$/i; columns.forEach((column) => { for (let rowIndex = monthStart; rowIndex < rows.length; rowIndex += 1) { const value = String(rows[rowIndex]?.[column.index] || '').trim(); const rawDate = String(rows[rowIndex]?.[0] || '').trim(); const monthYear = rawDate.match(/^([A-Za-z]{3,9})\s+(\d{2})$/); const parsedDate = monthYear ? new Date(`${monthYear[1]} 1, 20${monthYear[2]}`) : new Date(rawDate); const date = Number.isNaN(parsedDate.getTime()) ? rawDate : `${String(parsedDate.getMonth() + 1).padStart(2, '0')}/15/${parsedDate.getFullYear()}`; if (/^leased$/i.test(rawDate) || !monthYear || Number.isNaN(parsedDate.getTime()) || !value) continue; const numeric = value.replace(/[^0-9.-]/g, ''); const amount = numeric && Number.isFinite(Number(numeric)) ? Number(numeric) : null; if (amount !== null) { latest[column.unit] = amount; records.push({ unit: column.unit, address: column.address, date, amount, status: 'Collected' }); } else if (/^HK$/i.test(value) && latest[column.unit] !== undefined) { records.push({ unit: column.unit, address: column.address, date, amount: latest[column.unit], status: 'Autopay' }); } else if (zeroStatuses.test(value)) { records.push({ unit: column.unit, address: column.address, date, amount: 0, status: value }); } } }); return { records, latest }; };
const mergeOccupancyData = (current, text, unifiedText) => { const parsed = parseOccupancyCsv(text); const unifiedRows = parseUnifiedCsv(unifiedText || ''); const byUnit = new Map(unifiedRows.map((row) => [unitKey(row['NEW OCCUPANCY UNIT']), row]).filter(([key]) => key)); const byUnique = new Map((current.apartments || []).map((item) => [unitKey(item.uniqueId), item]).filter(([key]) => key)); const byOccupancy = new Map((current.apartments || []).map((item) => [unitKey(item.unifiedOccupancyUnit), item]).filter(([key]) => key)); const matched = new Map(); const recordApartment = new Map(); parsed.records.forEach((record) => { const row = byUnit.get(unitKey(record.unit)); const apartment = row ? (byUnique.get(unitKey(row['EASYPROD - UniquID'])) || byOccupancy.get(unitKey(record.unit))) : byOccupancy.get(unitKey(record.unit)); if (!apartment) return; const rows = matched.get(apartment.id) || []; if (!rows.some((item) => item.date === record.date)) rows.push(record); matched.set(apartment.id, rows); recordApartment.set(record, apartment); }); const apartments = current.apartments.map((item) => matched.has(item.id) ? { ...item, occupancyRentHistory: matched.get(item.id), rent: parsed.latest[unitKey(item.unifiedOccupancyUnit)] ?? item.rent } : item); const transactions = [...(current.transactions || []).filter((item) => !String(item.id || '').startsWith('occupancy-rent-'))]; const ids = new Set(); parsed.records.forEach((record) => { const apartment = recordApartment.get(record); if (!apartment) return; const id = `occupancy-rent-${apartment.id}-${record.date}`; if (ids.has(id)) return; ids.add(id); transactions.push({ id, apartmentId: apartment.id, Unit: record.unit, Description: transactionDate(record.date) <= new Date() ? 'Rent received' : 'Rent expected', Amount: record.amount, 'Order Date': record.date, Note: record.status }); }); return { ...current, apartments, transactions }; };
...
const renterHeader = (value) => String(value || '').replace(/^\uFEFF/, '').replace(/["*]/g, '').replace(/\s+/g, '').trim();
const parseDelimitedRecords = (text, delimiter, preserveEmpty = false) => { const records = []; let row = [], cell = '', quoted = false; for (let i = 0; i < text.length; i += 1) { const char = text[i]; if (char === '"') { if (quoted && text[i + 1] === '"') { cell += '"'; i += 1; } else quoted = !quoted; } else if (!quoted && char === delimiter) { row.push(cell); cell = ''; } else if (!quoted && (char === '\n' || char === '\r')) { if (char === '\r' && text[i + 1] === '\n') i += 1; row.push(cell); if (preserveEmpty || row.some((value) => value.trim())) records.push(row); row = []; cell = ''; } else cell += char; } if (cell || row.length) { row.push(cell); records.push(row); } return records; };
const parseRentersCsv = (text) => {
...
const renterApartmentMatch = (apartments, row) => { const combined = String(row['מספר'] || '').trim(); const parts = combined.split('/').map((value) => String(value || '').trim()); const street = String(row['רחוב'] || '').trim().replace(/\s+/g, ' '); const attempts = parts.length > 1 ? [{ building: parts[0], apartment: parts[1] }, { building: parts[1], apartment: parts[0] }] : [{ building: combined, apartment: '' }]; return attempts.flatMap(({ building, apartment }) => apartments.filter((item) => String(item.street || '').trim().replace(/\s+/g, ' ') === street && unitKey(item.streetNumber) === unitKey(building) && (!apartment || unitKey(item.number || item.appt || item.apptId) === unitKey(apartment)))).concat(apartments.filter((item) => unitKey(item.uniqueId) === unitKey(row['יחידה'])))[0]; };
...
function LoginLanding({ loginOpen, setLoginOpen, loginError, signIn }) {
  return <main className="login-landing"><div className="login-card"><img src="https://fivefortyinv.co.il/favicon.ico" alt="540 Investments" /><div className="login-brand">540 Investments<span>HASHKAOT DAROM</span></div><p>Property operations workspace</p><button className="primary login-button" onClick={() => setLoginOpen(true)}>Sign in</button></div>{loginOpen && <div className="login-overlay" onClick={() => setLoginOpen(false)}><form className="login-modal" onSubmit={signIn} onClick={(event) => event.stopPropagation()}><button type="button" className="login-close" onClick={() => setLoginOpen(false)}>×</button><h2>Sign in</h2><p>Welcome to 540 Investments</p><label>Username<input name="username" autoComplete="username" autoFocus /></label><label>Password<input name="password" type="password" autoComplete="current-password" /></label>{loginError && <div className="login-error">{loginError}</div>}<button className="primary" type="submit">Sign in</button></form></div>}</main>;
}
function App() {
  const [authenticated, setAuthenticated] = useState(() => localStorage.getItem('property-authenticated') === 'true');
  ...
  const [cloudLoadRequested, setCloudLoadRequested] = useState(false);
  ...
  const loadAllCloudData = () => { cloudDataLoaded.current = false; occupancyLoaded.current = false; setCloudSourceStatus({}); setCloudLoadRequested(false); setTimeout(() => setCloudLoadRequested(true), 0); };
  const loadCloudSource = async (source) => {
    const requiredBefore = { master: ['unified'], coordinates: ['unified', 'master'], renters: ['unified', 'master'], transactions: ['unified', 'master'], occupancy: ['unified', 'master', 'renters', 'transactions'] }[source] || [];
    const missing = requiredBefore.filter((name) => cloudSourceStatus[name]?.state !== 'ready');
    if (missing.length) { setCloudSourceStatus((current) => ({ ...current, [source]: { state: 'error', message: `Load first: ${missing.join(', ')}` } })); return; }
    ...
    try {
      ...
      if (source === 'unified') { setSelected(undefined); setTopSelected(null); setData((current) => createUnifiedFirstData(text, { ...current, buildings: [], apartments: [], transactions: [], people: [] })); }
      if (source === 'master') setData((current) => enrichUnifiedWithMaster(current, text));
  ...
  useEffect(() => { if (!authenticated || cloudDataLoaded.current || !cloudLoadRequested) return; cloudDataLoaded.current = true; setCloudDataStatus('Loading cloud spreadsheets…'); fetch('/api/data').then((response) => { if (!response.ok) throw new Error('Cloud spreadsheet API unavailable'); return response.json(); }).then(({ files }) => { cloudFilesRef.current = files; setData((current) => { let next = createUnifiedFirstData(files.unified || '', { ...current, buildings: [], apartments: [], transactions: [], people: [] }); next = enrichUnifiedWithMaster(next, files.master || ''); const incomingTransactions = parseTransactionsCsv(files.transactions || ''); const transactions = [...(next.transactions || [])]; incomingTransactions.forEach((item) => { const apartment = next.apartments.find((candidate) => transactionMatchesUniqueId(item, candidate)) || next.apartments.find((candidate) => transactionMatchesApartment(item, candidate)); const linked = { ...item, apartmentId: apartment?.id || '' }; if (!transactions.some((existing) => existing.PurchaseId === linked.PurchaseId)) transactions.push(linked); }); next = { ...next, transactions }; const apartments = next.apartments.map((item) => ({ ...item })); const people = [...(next.people || [])]; parseRentersCsv(files.renters || '').forEach((row, index) => { const target = renterApartmentMatch(apartments, row); if (!target) return; const tenantNames = String(row['שמות'] || '').split(/[;,、]+/).map((name) => name.trim()).filter(Boolean); const owner = String(row['בעל הדירה'] || '').trim(); Object.assign(target, { renterUnit: String(row['יחידה'] || '').trim(), tenantName: tenantNames.join(', '), resident: tenantNames.join(', '), ownerName: owner || target.ownerName, status: tenantNames.length ? 'Leased' : target.status, leaseStart: row['תחילתחוזה'] || target.leaseStart, leaseEnd: row['סיוםחוזה'] || target.leaseEnd, rent: Number(String(row['שכר דירה'] || row['שכרדירה'] || '').replace(/[^0-9.-]/g, '') || target.rent || 0), renterCsv: { ...row } }); }); return { ...next, apartments, people, transactions }; }); setCloudDataStatus('Cloud spreadsheets loaded'); }).catch((error) => { console.warn('[Cloud spreadsheets]', error.message); setCloudDataStatus('Cloud spreadsheets unavailable — using local data'); }); }, [authenticated, cloudLoadRequested]);

  // Coordinates are read from the R2 cache before any geocoding work is allowed to run.
  useEffect(() => { if (!authenticated || cloudDataStatus !== 'Cloud spreadsheets loaded') return undefined; const rows = parseCoordinateCsv(cloudFilesRef.current?.coordinates || ''); if (rows.length) setData((current) => ({ ...current, buildings: applyCoordinateCache(current.buildings, rows) })); return undefined; }, [authenticated, cloudDataStatus]);
  useEffect(() => { if (!authenticated || cloudDataStatus !== 'Cloud spreadsheets loaded' || occupancyLoaded.current) return undefined; occupancyLoaded.current = true; const occupancyText = cloudFilesRef.current?.occupancy || ''; if (occupancyText) setData((current) => mergeOccupancyData(current, occupancyText, cloudFilesRef.current?.unified || '')); return undefined; }, [authenticated, cloudDataStatus]);
  useEffect(() => localStorage.setItem('blockwise-data', JSON.stringify(withoutPhotoBlobs(data))), [data]);
  useEffect(() => {
    const normalized = [];
    const seen = new Set();
  ...
  useEffect(() => {
    ...
    geocodeBuildings(geocodeQueue, (buildings) => {
      ...
      if (!cancelled) { setData((current) => { const next = { ...current, buildings }; localStorage.setItem('blockwise-data', JSON.stringify(withoutPhotoBlobs(next))); return next; }); setGeocodeQueue(null); }
  ...
  const ownerTotals = data.apartments.reduce((totals, apartment) => { const owner = canonicalOwner(apartment.ownerContract || apartment.ownerName || apartment.internalOwner).toLowerCase(); if (owner === 'hd') totals.hd += 1; else if (owner === 'ka' || owner.includes('רוקון נאמנים')) totals.ka += 1; return totals; }, { hd: 0, ka: 0 });
  const saveRecord = (e) => {
    e.preventDefault();
  ...
  const importCsv = (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const imported = splitImportedBuildingsByEntrance(parseMasterCsv(String(reader.result || ''))); if (!imported || !Array.isArray(imported.buildings) || !Array.isArray(imported.apartments)) throw new Error('CSV did not produce building and apartment records'); const current = data && Array.isArray(data.buildings) && Array.isArray(data.apartments) ? data : hydrateData(); const nextData = mergeMasterCsv(current, imported); const normalized = { ...nextData, buildings: nextData.buildings || [], apartments: nextData.apartments || [], people: nextData.people || [], bills: nextData.bills || [], events: nextData.events || [], maintenance: nextData.maintenance || [] }; localStorage.setItem('blockwise-data', JSON.stringify(withoutPhotoBlobs(normalized))); setData(normalized); setGeocodeQueue(normalized.buildings); } catch (error) { console.error('CSV import failed', error); window.alert(`Could not import CSV: ${error.message}`); } }; reader.onerror = () => window.alert('Could not read this CSV file. Save it as UTF-8 CSV and try again.'); reader.readAsText(file, 'UTF-8'); e.target.value = ''; };
  const importRenters = (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const rows = parseRentersCsv(String(reader.result || '')); setData((current) => { const apartments = [...current.apartments]; const people = [...(current.people || [])]; let matched = 0; rows.forEach((row, index) => { const combined = String(row['מספר'] || '').trim(); const [buildingNumber, apartmentNumber] = combined.split('/').map((value) => String(value || '').trim()); const street = String(row['רחוב'] || '').trim(); const apartment = apartments.find((item) => String(item.street || '').trim() === street && String(item.streetNumber || '').trim() === buildingNumber && unitKey(item.number || item.appt || item.apptId) === unitKey(apartmentNumber)) || apartments.find((item) => String(item.street || '').trim() === street && String(item.streetNumber || '').trim() === buildingNumber && unitKey(item.number || item.appt || item.apptId) === unitKey(combined)); if (!apartment) return; matched += 1; const tenantName = String(row['שמות'] || '').trim(); const ownerName = String(row['בעל הדירה'] || '').trim(); const tenantPeople = tenantName.split(/[;,、]+/).map((name) => name.trim()).filter(Boolean); const ownerPeople = ownerName.split(/[;,、]+/).map((name) => name.trim()).filter(Boolean); const updatePerson = (name, role) => { const key = `${role}-${name.toLowerCase()}-${row['ת.ז'] || ''}`; let person = people.find((item) => item.importKey === key || (item.role === role && String(item.name || item.fullName || '').toLowerCase() === name.toLowerCase() && String(item.idNumber || '') === String(row['ת.ז'] || ''))); if (!person) { person = { id: `renter-person-${index}-${role}-${people.length}`, importKey: key, name, fullName: name, role, idNumber: row['ת.ז'] || '', phone: row['טלפונים'] || '', email: row['כתובת אימייל'] || '', permanentAddress: row['כתובת קבועה'] || '', apartmentIds: [] }; people.push(person); } person.apartmentIds = [...new Set([...(person.apartmentIds || []), apartment.id])]; return person; }; tenantPeople.forEach((name) => updatePerson(name, 'Tenant')); ownerPeople.forEach((name) => updatePerson(name, 'Owner')); const tenant = tenantPeople.join(', '); apartments[apartments.indexOf(apartment)] = { ...apartment, ownerName: ownerName || apartment.ownerName, ownerNames: [...new Set([...(apartment.ownerNames || []), ownerName].filter(Boolean))], tenantName: tenant || apartment.tenantName, resident: tenant || apartment.resident, status: tenant ? 'Leased' : apartment.status, leaseStart: row['תחילתחוזה'] || apartment.leaseStart || '', leaseEnd: row['סיוםחוזה'] || apartment.leaseEnd || '', rent: Number(String(row['שכרדירה'] || '').replace(/[^0-9.-]/g, '') || apartment.rent || 0), renterCsv: { unit: row['יחידה'] || '', owner: ownerName, idNumber: row['ת.ז'] || '', phone: row['טלפונים'] || '', email: row['כתובת אימייל'] || '', permanentAddress: row['כתובת קבועה'] || '', status: row['סטאטוס'] || '' } }; }); window.console.info(`[Renters CSV] ${rows.length} rows read; ${matched} apartments matched by street, building number, and apartment number.`); return { ...current, apartments, people }; }); } catch (error) { console.error('Renters CSV import failed', error); window.alert(`Could not import renters CSV: ${error.message}`); } }; reader.readAsText(file, 'UTF-8'); e.target.value = ''; };
  ...
  const importPhotosZip = async (event) => { const file = event.target.files?.[0]; if (!file) return; try { const zip = await JSZip.loadAsync(file); const entries = Object.values(zip.files).filter((entry) => !entry.dir && /\.(jpe?g|png|gif|webp|heic)$/i.test(entry.name)); const matches = new Map(); const apartmentUnits = data.apartments.flatMap((apartment) => String(apartment.renterUnit || apartment.renterCsv?.unit || '').split('/').map((unit) => [unitKey(unit), apartment]).filter(([unit]) => unit)); const unitFolderPattern = /^יחידה\s*[-_:]?\s*/i; setPhotoProgress({ done: 0, total: entries.length }); for (let index = 0; index < entries.length; index += 1) { const entry = entries[index]; // Search every path segment, so an extra ZIP folder level is harmless.
      const folder = entry.name.split('/').map((part) => part.trim()).find((part) => unitFolderPattern.test(part)); const unit = folder ? unitKey(folder.replace(unitFolderPattern, '')) : ''; const apartment = apartmentUnits.find(([candidate]) => candidate === unit)?.[1]; if (apartment) { const blob = await entry.async('blob'); const url = URL.createObjectURL(blob); matches.set(apartment.id, [...(matches.get(apartment.id) || []), { name: entry.name.split('/').pop(), url, unit }]); } setPhotoProgress({ done: index + 1, total: entries.length }); } setData((current) => ({ ...current, apartments: current.apartments.map((apartment) => matches.has(apartment.id) ? { ...apartment, photoUrls: [...(apartment.photoUrls || []), ...matches.get(apartment.id)] } : apartment) })); window.alert(`Photos imported: ${[...matches.values()].reduce((total, items) => total + items.length, 0)} matched across ${matches.size} apartments.`); } catch (error) { console.error('Photo ZIP import failed', error); window.alert(`Could not import photo ZIP: ${error.message}`); } finally { setPhotoProgress(null); event.target.value = ''; } };
  const syncPhotos = async () => { if (photoProgress) return; const units = [...new Set(data.apartments.flatMap((apartment) => String(apartment.renterUnit || apartment.renterCsv?.unit || apartment.unifiedZehavitUnit || '').split('/').map((unit) => unit.trim()).filter(Boolean)))]; if (!units.length) { setCloudDataStatus('No apartment yechida values available for photo sync'); return; } setPhotoProgress({ done: 0, total: units.length }); const available = {}; for (let index = 0; index < units.length; index += 1) { const unit = units[index]; try { const response = await fetch(`/api/photos?unit=${encodeURIComponent(unit)}`); const result = response.ok ? await response.json() : { photos: [] }; available[unitKey(unit)] = Boolean((result.photos || []).length); } catch { available[unitKey(unit)] = false; } setPhotoProgress({ done: index + 1, total: units.length }); } setPhotoAvailability(available); setPhotoProgress(null); setCloudDataStatus(`Photos synced for ${units.length} apartment units`); setTimeout(() => setCloudDataStatus(''), 5000); };
  ...
  const topResults = search.trim()
    ...
    : [];
  const highlight = (value) => { const text = String(value || ''); const query = search.trim(); if (!query) return text; const escaped = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); return text.split(new RegExp(`(${escaped})`, 'ig')).map((part, index) => part.toLowerCase() === query.toLowerCase() ? <mark key={index}>{part}</mark> : part); };
  const signIn = (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); if (String(form.get('username')).trim().toLowerCase() === 'elie' && String(form.get('password')) === 'HD') { localStorage.setItem('property-authenticated', 'true'); setAuthenticated(true); setLoginOpen(false); setLoginError(''); } else setLoginError('Incorrect username or password.'); };
  const logout = () => { localStorage.removeItem('property-authenticated'); setAuthenticated(false); setTopSelected(null); };
  if (!authenticated) return <LoginLanding loginOpen={loginOpen} setLoginOpen={setLoginOpen} loginError={loginError} signIn={signIn} />;
  return (
    <div className="app">
      <aside>
        <div className="brand">
          ...
          <div>
            540 Investments<span>HASHKAOT DAROM</span>
          </div>
        </div>
      ...
      <main>
        <header>
          <div>
            ...
            <h1>{tab === 'Overview' ? 'Welcome Elie' : tab}</h1>
            <p>
              {tab === 'Overview'
          ...
          <div className="actions top-search-wrap">
            ...
            <button className="outline small" onClick={regenerateCoordinates} disabled={Boolean(coordinateProgress)}>Export real coordinates</button>
            ...
            <button className="outline small" onClick={loadAllCloudData} disabled={cloudDataLoading}>Load all cloud data</button>
            ...
            {topResults.length > 0 && (
              <div className="top-search-results">
                {topResults.map(({ type, item }) => (
                  ...
                  >
                    {type === 'person' ? (
                      <><b>{highlight(item.name)}</b><small>{highlight(item.role)} · {item.phone || 'No phone'} · {item.email || 'No email'}</small></>
                    ) : type === 'building' ? (
                      <>
                        <b>{highlight(item.name)}</b>
                        <small>{highlight(item.address || item.area)} · Building</small>
                      </>
                    ) : (
                      <>
                        <b>{highlight(item.renterUnit || item.renterCsv?.unit || item.number)}</b>
                        <small>
        ...
        {tab === 'Overview' ? (
          <>
            <section className="metrics">
              <div className="metric portfolio-summary"><span>Portfolio</span><div><strong>{data.buildings.length}</strong><small>Buildings</small></div><div><strong>{data.apartments.length}</strong><small>Apartments</small></div><em>{occupied} occupied · {data.apartments.length ? `${Math.round((occupied / data.apartments.length) * 100)}%` : '0%'} overall occupancy</em></div>
              <div className="metric ownership-summary"><span>Ownership & occupancy</span><div><strong>{ownerTotals.hd}</strong><small>HD apartments</small><em>{data.apartments.filter((a) => canonicalOwner(a.ownerContract || a.ownerName || a.internalOwner).toLowerCase() === 'hd' && a.status === 'Leased').length} leased</em></div><div><strong>{ownerTotals.ka}</strong><small>KA apartments</small><em>{data.apartments.filter((a) => ['ka'].includes(canonicalOwner(a.ownerContract || a.ownerName || a.internalOwner).toLowerCase()) || String(a.ownerContract || a.ownerName || '').includes('רוקון נאמנים')).filter((a) => a.status === 'Leased').length} leased</em></div></div>
            </section>
            <section className="grid">
              <RealMap buildings={visibleBuildings} selected={selected} setSelected={setSelected} geocoding={Boolean(geocodeQueue) || Boolean(coordinateProgress)} />
              <Detail
                building={selected}
                ...
                onApartment={(apartment) => setTopSelected({ type: 'apartment', item: apartment })}
              />
            </section>
            <section className="bottom-grid">
              <ApartmentTable apartments={data.apartments} people={data.people} buildings={data.buildings} onApartment={(apartment) => setTopSelected({ type: 'apartment', item: apartment })} onPerson={(person) => setTopSelected({ type: 'person', item: person })} onBuilding={(building) => setTopSelected({ type: 'building', item: building })} />
...
function LegacyDataSpreadsheet({ data, onApartment }) {
  const [query, setQuery] = useState('');
  const [ownerFilter, setOwnerFilter] = useState('All');
  const [statusFilter, setStatusFilter] = useState('All');
  const groupOf = (apartment) => { const value = String(apartment.groupOwner || apartment.leaseOwner || apartment.ownerName || apartment.internalOwner || '').toLowerCase(); if (value.includes('weiss') || value.includes('השקעות דרום') || value.includes('וייס') || value === 'hd' || value === 'zw') return 'Weiss'; if (value.includes('morris') || value.includes('רוקון') || value.includes('קרן אבירם') || value.includes('מוריס') || ['ka', 'rc', 'mdd'].includes(value)) return 'Morris'; if (value.includes('amidar') || value.includes('עמידר')) return 'Amidar'; return apartment.groupOwner || apartment.leaseOwner || apartment.ownerName || apartment.internalOwner || 'Other'; };
  const rows = data.apartments.map((apartment) => {
    const building = data.buildings.find((item) => item.id === apartment.buildingId);
    const renterRecords = apartment.renterRecords?.length ? apartment.renterRecords : [apartment.renterCsv || {}];
    const units = [...new Set(renterRecords.map((record) => record.unit).filter(Boolean))].join(' / ') || apartment.renterUnit || '—';
    const tenants = [...new Set(renterRecords.flatMap((record) => record.tenantNames || []).filter(Boolean))].join(' / ') || apartment.tenantName || '—';
    const transactions = (data.transactions || []).filter((item) => item.apartmentId === apartment.id || transactionMatchesApartment(item, apartment));
    const byType = transactions.reduce((result, transaction) => { const type = transaction.Description || transaction.Vendor || 'Other'; result[type] = (result[type] || 0) + Number(String(transaction.Amount || 0).replace(/[^0-9.-]/g, '') || 0); return result; }, {});
    const owner = apartment.leaseOwner || apartment.ownerName || apartment.internalOwner || '—';
    const group = groupOf(apartment);
    const status = tenants === '—' || tenants.includes('אין דייר') ? 'Vacant' : (apartment.leaseEnd && new Date(apartment.leaseEnd) <= new Date(Date.now() + 60 * 86400000) ? 'About to be vacant' : 'Leased');
    return { apartment, building, units, tenants, owner, group, status, byType, transactions };
  }).filter((row) => { const text = `${row.units} ${row.apartment.uniqueId} ${row.apartment.apptId} ${row.apartment.number} ${row.building?.name || ''} ${row.owner} ${row.group} ${row.tenants} ${row.status}`.toLowerCase(); return (ownerFilter === 'All' || row.group === ownerFilter) && (statusFilter === 'All' || row.status === statusFilter) && text.includes(query.toLowerCase()); }).sort((a, b) => String(a.units).localeCompare(String(b.units), undefined, { numeric: true }));
  return <section className="panel data-spreadsheet"><div className="panel-head spreadsheet-head"><div><h2>Data Spreadsheet</h2><p>{rows.length} apartment records · synchronized with uploaded CSV data</p></div><div className="spreadsheet-controls"><input className="search" placeholder="Search any apartment, owner, tenant, ID…" value={query} onChange={(event) => setQuery(event.target.value)} /><select value={ownerFilter} onChange={(event) => setOwnerFilter(event.target.value)}><option>All owners</option><option>Weiss</option><option>Morris</option><option>Amidar</option><option>Other</option></select><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option>All statuses</option><option>Leased</option><option>Vacant</option><option>About to be vacant</option></select></div></div><div className="spreadsheet-scroll"><table><thead><tr><th>UniqueID</th><th>Unit / יחידה</th><th>Address</th><th>Other addresses</th><th>Lease owner</th><th>Group owner</th><th>Tenant / contact</th><th>Status</th><th>Lease dates</th><th>Rent</th><th>Transactions / bills by type</th></tr></thead><tbody>{rows.map((row) => <tr key={row.apartment.id} onClick={() => onApartment(row.apartment)}><td>{row.apartment.uniqueId || '—'}<small>{row.apartment.apptId || '—'}</small></td><td><b>{row.units}</b><small>Apt {row.apartment.number || '—'} · Floor {row.apartment.floor || '—'}</small></td><td>{row.building?.name || '—'}<small>{row.apartment.addressNotEditable || row.building?.address || '—'}</small></td><td>{[row.apartment.altAddress, ...row.transactions.map((item) => item.address)].filter(Boolean).join(' · ') || '—'}</td><td>{row.owner}</td><td><span className={`spreadsheet-group ${row.group.toLowerCase()}`}>{row.group}</span></td><td>{row.tenants}<small>{row.apartment.renterCsv?.phone || '—'} · {row.apartment.renterCsv?.email || '—'}</small></td><td>{row.status}</td><td>{row.apartment.leaseStart || '—'} → {row.apartment.leaseEnd || '—'}</td><td>{money(row.apartment.rent || 0)}</td><td>{Object.entries(row.byType).map(([type, amount]) => <span className="spreadsheet-cost" key={type}>{type}: {money(amount)}</span>)}{!Object.keys(row.byType).length && '—'}</td></tr>)}</tbody></table>{!rows.length && <p className="empty">No matching apartment records.</p>}</div></section>;
}
function DataSpreadsheet({ data, onApartment }) {
  const [filters, setFilters] = useState({ uniqueId: '', apptId: '', unit: '', address: '', owner: '', tenant: '', status: '' });
  const ownerGroup = (value) => { const text = String(value || '').trim().toLowerCase(); if (['hd', 'zw'].includes(text) || text.includes('השקעות דרום') || text.includes('וייס')) return 'Weiss'; if (['ka', 'rc', 'mdd'].includes(text) || text.includes('רוקון') || text.includes('קרן אבירם') || text.includes('מוריס')) return 'Morris'; if (text.includes('amidar') || text.includes('עמידר')) return 'Amidar'; return value || 'Other'; };
  const rows = data.apartments.flatMap((apartment) => {
    const records = apartment.renterRecords?.length ? apartment.renterRecords : [{ unit: apartment.renterUnit || apartment.renterCsv?.unit || '', tenantNames: String(apartment.tenantName || '').split(',').map((name) => name.trim()).filter(Boolean), owner: apartment.renterCsv?.owner, phone: apartment.renterCsv?.phone, email: apartment.renterCsv?.email, leaseStart: apartment.leaseStart, leaseEnd: apartment.leaseEnd, rent: apartment.rent }];
    return records.length ? records.map((record, index) => ({ apartment, record, rowId: `${apartment.id}-${record.unit || index}` })) : [{ apartment, record: {}, rowId: apartment.id }];
  }).map(({ apartment, record, rowId }) => {
    const building = data.buildings.find((item) => item.id === apartment.buildingId);
    const owner = record.owner || apartment.renterCsv?.owner || apartment.leaseOwner || apartment.ownerName || apartment.internalOwner || 'Not recorded';
    const tenants = (record.tenantNames || []).filter(Boolean).join(' / ') || apartment.tenantName || '—';
    const transactions = (data.transactions || []).filter((item) => item.apartmentId === apartment.id || transactionMatchesApartment(item, apartment));
    const transactionTypes = transactions.reduce((result, item) => { const type = item.Description || item.Vendor || 'Other'; result[type] = (result[type] || 0) + Number(String(item.Amount || 0).replace(/[^0-9.-]/g, '') || 0); return result; }, {});
    const bills = (data.bills || []).filter((bill) => bill.apartmentId === apartment.id).reduce((result, bill) => { const type = bill.category || 'Bill'; result[type] = (result[type] || 0) + Number(bill.amount || 0); return result; }, {});
    const end = record.leaseEnd || apartment.leaseEnd;
    const status = !tenants || tenants === '—' || tenants.includes('אין דייר') ? 'Vacant' : end && new Date(end) <= new Date(Date.now() + 60 * 86400000) ? 'About to be vacant' : 'Leased';
    return { apartment, record, rowId, building, owner, group: ownerGroup(owner), tenants, status, transactionTypes: { ...transactionTypes, ...bills } };
  }).filter((row) => Object.entries(filters).every(([field, value]) => !value || String(field === 'uniqueId' ? row.apartment.uniqueId : field === 'apptId' ? row.apartment.apptId : field === 'unit' ? row.record.unit : field === 'address' ? `${row.building?.name || ''} ${row.building?.address || ''} ${row.apartment.altAddress || ''}` : field === 'owner' ? `${row.owner} ${row.group}` : field === 'tenant' ? row.tenants : row.status).toLowerCase().includes(value.toLowerCase()))).sort((a, b) => String(a.record.unit || '').localeCompare(String(b.record.unit || ''), undefined, { numeric: true, sensitivity: 'base' }));
  const exportCsv = () => { const headers = ['UniqueID', 'Appt ID', 'Unit / Yechida', 'Building', 'Address', 'Other addresses', 'Lease owner', 'Group owner', 'Tenant names', 'Tenant phone', 'Tenant email', 'Status', 'Lease start', 'Lease end', 'Rent', 'Transactions / bills by type']; const esc = (value) => `"${String(value ?? '').replace(/"/g, '""')}"`; const csv = [headers, ...rows.map((row) => [row.apartment.uniqueId, row.apartment.apptId, row.record.unit, row.building?.name, row.building?.address || row.apartment.addressNotEditable, row.apartment.altAddress, row.owner, row.group, row.tenants, row.record.phone || row.apartment.renterCsv?.phone, row.record.email || row.apartment.renterCsv?.email, row.status, row.record.leaseStart || row.apartment.leaseStart, row.record.leaseEnd || row.apartment.leaseEnd, row.record.rent || row.apartment.rent, Object.entries(row.transactionTypes).map(([type, amount]) => `${type}: ${amount}`).join(' | ')])].map((line) => line.map(esc).join(',')).join('\r\n'); const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); link.download = 'property-data-spreadsheet.csv'; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000); };
  const setFilter = (field, value) => setFilters((current) => ({ ...current, [field]: value }));
  return <section className="panel data-spreadsheet"><div className="panel-head spreadsheet-head"><div><h2>Data Spreadsheet</h2><p>{rows.length} apartment / יחידה rows · synchronized from all uploaded CSVs</p></div><button className="outline small" onClick={exportCsv}>Export CSV</button></div><div className="spreadsheet-field-filters"><input placeholder="UniqueID" value={filters.uniqueId} onChange={(e) => setFilter('uniqueId', e.target.value)} /><input placeholder="Appt ID" value={filters.apptId} onChange={(e) => setFilter('apptId', e.target.value)} /><input placeholder="Unit / יחידה" value={filters.unit} onChange={(e) => setFilter('unit', e.target.value)} /><input placeholder="Address" value={filters.address} onChange={(e) => setFilter('address', e.target.value)} /><input placeholder="Lease / group owner" value={filters.owner} onChange={(e) => setFilter('owner', e.target.value)} /><input placeholder="Tenant" value={filters.tenant} onChange={(e) => setFilter('tenant', e.target.value)} /><input placeholder="Status" value={filters.status} onChange={(e) => setFilter('status', e.target.value)} /></div><div className="spreadsheet-scroll"><table><thead><tr><th>UniqueID</th><th>Appt ID</th><th>Unit / יחידה</th><th>Address</th><th>Other addresses</th><th>Lease owner</th><th>Group owner</th><th>Tenant</th><th>Status</th><th>Lease dates</th><th>Rent</th><th>Transactions / bills by type</th></tr></thead><tbody>{rows.map((row) => <tr key={row.rowId} onClick={() => onApartment(row.apartment)}><td>{row.apartment.uniqueId || '—'}</td><td>{row.apartment.apptId || '—'}</td><td><b>{row.record.unit || '—'}</b><small>Apt {row.apartment.number || '—'} · Floor {row.apartment.floor || '—'}</small></td><td>{row.building?.name || '—'}<small>{row.apartment.addressNotEditable || row.building?.address || '—'}</small></td><td>{row.apartment.altAddress || '—'}</td><td>{row.owner}</td><td><span className={`spreadsheet-group ${row.group.toLowerCase()}`}>{row.group}</span></td><td>{row.tenants}<small>{row.record.phone || row.apartment.renterCsv?.phone || '—'} · {row.record.email || row.apartment.renterCsv?.email || '—'}</small></td><td>{row.status}</td><td>{row.record.leaseStart || row.apartment.leaseStart || '—'} → {row.record.leaseEnd || row.apartment.leaseEnd || '—'}</td><td>{money(row.record.rent || row.apartment.rent || 0)}</td><td>{Object.entries(row.transactionTypes).map(([type, amount]) => <span className="spreadsheet-cost" key={type}>{type}: {money(amount)}</span>)}{!Object.keys(row.transactionTypes).length && '—'}</td></tr>)}</tbody></table>{!rows.length && <p className="empty">No matching records.</p>}</div></section>;
}
function MapExplorer({ data, selected, setSelected }) {
  const [mode, setMode] = useState('2D');
  const [externalView, setExternalView] = useState(null);
  const building = selected || data.buildings[0];
  const coordinates = building && Number.isFinite(Number(building.lat)) && Number.isFinite(Number(building.lng)) ? `${Number(building.lat)},${Number(building.lng)}` : '31.2639,34.7998';
  const streetViewUrl = `https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=${coordinates}`;
  const threeDUrl = `https://www.google.com/maps/@${coordinates},18z/data=!3m1!1e3`;
  const mapillaryUrl = `https://www.mapillary.com/app/?lat=${coordinates.split(',')[0]}&lng=${coordinates.split(',')[1]}&z=18`;
  const kartaviewUrl = `https://kartaview.org/map/@${coordinates.split(',')[0]},${coordinates.split(',')[1]},18z`;
  const viewUrl = externalView === 'street' ? streetViewUrl : threeDUrl;
  return <section className="map-explorer">
    <div className="map-explorer-toolbar"><div><h2>Interactive property map</h2><p>Explore the portfolio, open buildings, and jump to aerial or street-level views.</p></div><div className="map-mode-buttons"><button className={mode === '2D' ? 'active' : ''} onClick={() => setMode('2D')}>2D map</button><button className={mode === '3D' ? 'active' : ''} onClick={() => setMode('3D')}>3D / satellite</button><a href={threeDUrl} target="_blank" rel="noreferrer">Open 3D ↗</a><a href={streetViewUrl} target="_blank" rel="noreferrer">Google Street View ↗</a><a href={mapillaryUrl} target="_blank" rel="noreferrer">Mapillary ↗</a><a href={kartaviewUrl} target="_blank" rel="noreferrer">KartaView ↗</a></div></div>
    <div className="map-explorer-map"><RealMap buildings={data.buildings} selected={selected} setSelected={setSelected} /></div>
    {building && <div className="map-explorer-selected"><span className="eyebrow">SELECTED BUILDING</span><h3>{building.name}</h3><p>{building.address || `${building.street || ''} ${building.streetNumber || ''}`}</p><div><b>{building.units || data.apartments.filter((apartment) => apartment.buildingId === building.id).length}</b> apartments · <b>{building.floors || '—'}</b> floors</div><a href={streetViewUrl} target="_blank" rel="noreferrer">Open this building in Street View →</a></div>}
    {mode === '3D' && <div className="map-explorer-note">Satellite/3D imagery is provided by Google Maps without storing an API key in this app.</div>}
  </section>;
}
function Detail({ building, apartments, rent, data, onAdd, onOpen, onApartment }) {
  ...
  return (
    <div className="panel detail-panel">
      ...
      {data && <BuildingFinanceSummary building={building} apartments={apartments} data={data} onApartment={onApartment} />}
...
function BuildingFinanceSummary({ building, apartments, data, onApartment }) {
  ...
  const rentRows = apartments.flatMap((apartment) => (apartment.occupancyRentHistory || []).map((record) => ({ apartment, record })));
  ...
  return <div className="building-finance-summary"><div className="building-finance-totals"><span className="spent"><b>↓ {money(spent)}</b><small>Total spent</small></span><span className="collected"><b>↑ {money(collected)}</b><small>Total collected</small></span></div><div className="building-cashflow"><div className="cashflow-bars">{months.map((item) => <button title={`${item.label}: ${money(item.net)}`} key={item.key} style={{ height: `${Math.max(4, Math.round(Math.abs(item.net) / max * 100))}%` }} className={item.net >= 0 ? 'positive' : 'negative'} />)}</div><small>Cash flow over the last 3 years · {building.name}</small></div><h3>Individual rent receipts</h3><div className="building-transaction-list">{rentRows.sort((a, b) => transactionDate(b.record.date) - transactionDate(a.record.date)).map(({ apartment, record }, index) => <button type="button" className="building-transaction" key={`${apartment.id}-${record.date}-${index}`} onClick={() => onApartment?.(apartment)}><span>↑ {record.date} · Apt {apartment.number || apartment.apptId || '—'} · Yechida {record.unit}</span><b>{money(record.amount || 0)}</b></button>)}{!rentRows.length && <small className="muted">No occupancy rents recorded.</small>}</div><h3>Recent transactions</h3><div className="building-transaction-list">{transactions.map((item) => <div className="building-transaction" key={item.id || item.PurchaseId}><span>{item['Order Date'] || '—'} · {item.Description || item.Vendor || 'Transaction'}</span><b>{money(Number(String(item.Amount || 0).replace(/[^0-9.-]/g, '') || 0))}</b></div>)}{!transactions.length && <small className="muted">No transactions recorded for this building.</small>}</div></div>;
}
function ApartmentTable({ apartments, people = [], buildings = [], onApartment, onPerson, onBuilding }) {
  const [statusFilter, setStatusFilter] = useState('All');
  const [ownerFilter, setOwnerFilter] = useState('All');
  const leaseStatus = (apartment) => {
    if (!apartment.tenantName && apartment.status !== 'Leased') return 'Vacant';
    const end = apartment.leaseEnd ? new Date(apartment.leaseEnd) : null;
    const days = end && !Number.isNaN(end.getTime()) ? (end - new Date()) / 86400000 : Infinity;
    return days >= 0 && days <= 60 ? 'Lease ending soon' : 'Leased';
  };
  return (
      ...
      <div className="table">
        ...
        {[...apartments].filter((a) => String(a.renterUnit || a.renterCsv?.unit || '').trim()).filter((a) => statusFilter === 'All' || leaseStatus(a) === statusFilter).filter((a) => { const owner = canonicalOwner(a.ownerContract || a.ownerName || a.internalOwner).toLowerCase(); return ownerFilter === 'All' || (ownerFilter === 'HD' && owner === 'hd') || (ownerFilter === 'KA' && (owner === 'ka' || owner.includes('רוקון נאמנים'))) || (ownerFilter === 'Other' && owner !== 'hd' && owner !== 'ka' && !owner.includes('רוקון נאמנים')); }).sort((a, b) => { const left = String(a.renterUnit || a.renterCsv?.unit || '').trim(); const right = String(b.renterUnit || b.renterCsv?.unit || '').trim(); const numeric = Number(left) - Number(right); return Number.isNaN(numeric) ? left.localeCompare(right, undefined, { numeric: true }) : numeric; }).map((a) => {
          ...
          return <div className="tr" key={a.id}>
            <button className="table-link" onClick={() => onApartment?.(a)}>{a.renterUnit || a.renterCsv?.unit}</button>
            <span>
...
function InvestedPropertiesViewUpdated({ data, people = [], photoAvailability = {}, onApartment, onPerson, onBuilding }) {
  const [status, setStatus] = useState('All');
  const [groupFilter, setGroupFilter] = useState('All');
  const [photoFilter, setPhotoFilter] = useState('All');
  const [layout, setLayout] = useState('building');
  const [query, setQuery] = useState('');
  const [period, setPeriod] = useState('all');
  const investedGroup = (a) => { const value = String([a.groupOwner, a.leaseOwner, a.ownerContract, a.ownerName, a.internalOwner].filter(Boolean).join(' ')).trim().toLowerCase(); if (['hd', 'zw'].some((alias) => new RegExp('(^|\\s)' + alias + '(?:\\s|$)', 'i').test(value)) || value.includes('weiss') || value.includes('השקעות דרום') || value.includes('וייס זאב דוד')) return 'Weiss'; if (['ka', 'rc', 'mdd'].some((alias) => new RegExp('(^|\\s)' + alias + '(?:\\s|$)', 'i').test(value)) || value.includes('morris') || value.includes('רוקון נאמנים') || value.includes('קון נאמנים') || value.includes('קרן אבירם') || value.includes('מוריס דויד דניאל')) return 'Morris'; return 'Other'; };
  const apartments = (data.apartments || []).filter((a) => a.renterUnit || a.renterCsv?.unit || a.renterRecords?.length);
  const statusOf = (a, r) => { const names = r.tenantNames?.length ? r.tenantNames : String(a.tenantName || '').split(',').map((n) => n.trim()).filter(Boolean); if (!names.length || names.some((n) => n.includes('אין דייר'))) return 'Vacant'; const end = r.leaseEnd || a.leaseEnd; const date = end ? new Date(end) : null; return date && !Number.isNaN(date.getTime()) && date >= new Date() && date <= new Date(Date.now() + 60 * 86400000) ? 'About to be vacant' : 'Leased'; };
  const hasPhotos = (a) => { const units = String(a.renterUnit || a.renterCsv?.unit || '').split('/').map((u) => unitKey(u.trim())); return Boolean(a.photoUrls?.length || units.some((u) => photoAvailability[u])); };
  const rows = apartments.flatMap((a) => { const records = a.renterRecords?.length ? a.renterRecords : [{ unit: a.renterUnit || a.renterCsv?.unit || '—', tenantNames: String(a.tenantName || '').split(',').map((n) => n.trim()).filter(Boolean), phone: a.renterCsv?.phone, email: a.renterCsv?.email, leaseStart: a.leaseStart, leaseEnd: a.leaseEnd, rent: a.rent }]; return records.map((record, index) => ({ apartment: a, record, key: a.id + '-' + (record.unit || index) })); }).filter(({ apartment: a, record: r }) => { const group = investedGroup(a); const rowStatus = statusOf(a, r); const text = [r.unit, a.uniqueId, a.apptId, a.number, a.floor, a.size, ...(r.tenantNames || []), a.leaseOwner, group, rowStatus].join(' ').toLowerCase(); return (groupFilter === 'All' || group === groupFilter) && (status === 'All' || rowStatus === status) && (photoFilter === 'All' || (photoFilter === 'With photos' ? hasPhotos(a) : !hasPhotos(a))) && text.includes(query.toLowerCase()); });
  const sortedRows = rows.slice().sort((a, b) => layout === 'unit' ? String(a.record.unit || '').localeCompare(String(b.record.unit || ''), undefined, { numeric: true }) : String(data.buildings.find((x) => x.id === a.apartment.buildingId)?.name || '').localeCompare(String(data.buildings.find((x) => x.id === b.apartment.buildingId)?.name || ''), undefined, { numeric: true }) || String(a.record.unit || '').localeCompare(String(b.record.unit || ''), undefined, { numeric: true }));
  const count = (group) => apartments.filter((a) => investedGroup(a) === group && (status === 'All' || statusOf(a, a.renterRecords?.[0] || {}) === status)).length;
  const personFor = (name, role, a) => onPerson(people.find((p) => p.role === role && String(p.name || '').toLowerCase() === String(name || '').toLowerCase()) || { name, role, apartmentIds: [a.id] });
  const exportCsv = () => { const headers = ['Unit / Yechida', 'UniqueID', 'Appt ID', 'Building', 'Address', 'Floor', 'Size', 'Lease owner', 'Group owner', 'Tenant', 'Phone', 'Email', 'Status', 'Lease start', 'Lease end', 'Rent', 'Photos']; const esc = (v) => '"' + String(v ?? '').replace(/"/g, '""') + '"'; const csv = [headers, ...sortedRows.map(({ apartment: a, record: r }) => [r.unit, a.uniqueId, a.apptId, data.buildings.find((b) => b.id === a.buildingId)?.name, a.addressNotEditable || a.address, a.floor, a.size, leaseOwnerName(a.leaseOwner || a.ownerName), investedGroup(a), (r.tenantNames || []).join(' / '), r.phone, r.email, statusOf(a, r), r.leaseStart || a.leaseStart, r.leaseEnd || a.leaseEnd, r.rent || a.rent, hasPhotos(a) ? 'Yes' : 'No'])].map((line) => line.map(esc).join(',')).join('\r\n'); const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); link.download = 'invested-apartments.csv'; link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000); };
  const renderRow = ({ apartment: a, record: r, key }) => { const group = investedGroup(a); const rowStatus = statusOf(a, r); const building = data.buildings.find((b) => b.id === a.buildingId); const owner = leaseOwnerName(a.leaseOwner || a.ownerName || 'Owner not recorded'); return <div className={'invested-row ' + ownerTone(group)} key={key}><button className="table-link unit-link" onClick={() => onApartment(a)}><b>{r.unit || '—'}</b><small>יחידה</small></button><span><button className="table-link" onClick={() => personFor(owner, 'Lease owner', a)}>{owner}</button><button className="table-link group-link" onClick={() => personFor(group, 'Group owner', a)}>Group: {group}</button></span><span><button className="table-link" onClick={() => onApartment(a)}><b>{a.uniqueId || '—'} / {a.apptId || '—'}</b></button><small>Apt {a.number || '—'} · Floor {a.floor || '—'} · {a.size || '—'} m²</small></span><span><span>Tenant</span>{r.tenantNames?.length ? r.tenantNames.map((name, i) => <button className="table-link tenant-link" key={name + i} onClick={() => personFor(name, 'Tenant', a)}>{name}</button>) : <small>Vacant</small>}<small>{r.phone || 'No phone'} · {r.email || 'No email'}</small></span><span><b>{rowStatus === 'Vacant' ? 'Vacant' : money(r.rent || a.rent || 0)}</b><small>{r.leaseStart || '—'} → {r.leaseEnd || '—'} · {rowStatus}</small></span><span className="invested-finance"><b>{hasPhotos(a) ? '📷 Photos' : 'No photos'}</b><small>{building?.name || '—'}</small></span></div>; };
  const sections = layout === 'unit' ? <div className="invested-rows">{sortedRows.map(renderRow)}</div> : [...new Set(sortedRows.map((row) => row.apartment.buildingId))].map((buildingId) => { const building = data.buildings.find((b) => b.id === buildingId); const buildingRows = sortedRows.filter((row) => row.apartment.buildingId === buildingId); return <section className="building-group" key={buildingId}><div className="group-heading"><h3><button className="invested-building-link" onClick={() => onBuilding?.(building)}>{building?.name || 'Building'}</button></h3><span>{buildingRows.length} records</span></div><div className="invested-rows">{buildingRows.map(renderRow)}</div></section>; });
  return <section className="panel list-view invested-properties"><div className="panel-head invested-header"><div><h2>Invested Properties</h2><p>{sortedRows.length} visible יחידה records</p><div className="invested-counts"><button className={groupFilter === 'All' ? 'active' : ''} onClick={() => setGroupFilter('All')}>All <small>{count('Weiss') + count('Morris')} apartments</small></button><button className={groupFilter === 'Weiss' ? 'active' : ''} onClick={() => setGroupFilter('Weiss')}>Weiss <small>{count('Weiss')} apartments</small></button><button className={groupFilter === 'Morris' ? 'active' : ''} onClick={() => setGroupFilter('Morris')}>Morris <small>{count('Morris')} apartments</small></button></div></div><div className="invested-controls"><input className="search" placeholder="Search unit, tenant, owner, ID…" value={query} onChange={(e) => setQuery(e.target.value)} /><select value={status} onChange={(e) => setStatus(e.target.value)}><option>All</option><option>Leased</option><option>Vacant</option><option>About to be vacant</option></select><select value={photoFilter} onChange={(e) => setPhotoFilter(e.target.value)}><option>All photos</option><option>With photos</option><option>Without photos</option></select><select value={layout} onChange={(e) => setLayout(e.target.value)}><option value="building">Show by building</option><option value="unit">Show by unit number</option></select><select value={period} onChange={(e) => setPeriod(e.target.value)}><option value="all">Costs/rent · All time</option><option value="month">Last month</option><option value="quarter">Last quarter</option><option value="half">Last 6 months</option><option value="year">Last year</option></select><button className="outline small" onClick={exportCsv}>Export apartments CSV</button></div></div>{sections}{!sortedRows.length && <p className="empty">No matching renter records.</p>}</section>;
}
    ...
    tab === 'Buildings'
      ...
      : tab === 'Apartments'
        ...
        : tab === 'People'
          ...
          : data.maintenance;
  return (
    <section className={`panel list-view ${tab === 'People' ? 'people-list-view' : ''}`}>
      <div className="panel-head">
        <div>
          <h2>{tab} register</h2>
        ...
        <div>
          ...
          {tab === 'Buildings' && (
            <>
              ...
              {tab === 'Buildings' && <button className="outline small" onClick={runAudit}>Scan apartments</button>}
              ...
              >
                ...
                {tab === 'Buildings' && (
                  <div className="building-apartments">
                    <a className="building-streetview-link tile-streetview" aria-label="Open Google Street View" title="Open Google Street View" href={googleStreetViewUrl(r)} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>🌐↗</a>
                    {data.apartments
                      .filter((apartment) => apartment.buildingId === r.id)
                      .sort(apartmentSort)
                      .map((apartment) => (
                        <span
                          className={`${ownerTone(apartment.ownerContract || apartment.internalOwner || apartment.ownerName)} ${apartment.status === 'Leased' ? 'apartment-leased' : ''}`}
                          key={apartment.id}
                          onClick={(event) => {
      ...
      {tab === 'Buildings' && audit && <div className="audit-report"><b>Apartment scan · {audit.ranAt}</b><p>{audit.missing.length ? `Missing sequential apartments: ${audit.missing.map((item) => `${item.building} (${item.address}) · ${item.apartment}`).join(' | ')}` : 'No sequential apartment gaps found.'}</p><p>{audit.conflicts.length ? `Conflicting records: ${audit.conflicts.map((item) => `${item.building} · Apt ${item.apartment} · ${item.versions.join(' VS ')}`).join(' | ')}` : 'No conflicting owner/tenant versions found.'}</p><p>{audit.incomplete.length ? `Incomplete records: ${audit.incomplete.map((item) => `${item.building} · Apt ${item.apartment} missing ${item.missing}`).join(' | ')}` : 'All scanned apartments have an owner and tenant record.'}</p></div>}
...
function BuildingDrawer({ building, apartments, onApartment, onClose }) {
  ...
  return (
    <aside className="side-drawer building-drawer" {...swipeHandlers}>
      ...
      <h2>{building.name}</h2>
      <a className="building-streetview-link" href={googleStreetViewUrl(building)} target="_blank" rel="noreferrer">Open Google Street View ↗</a>
      <p className="muted">
            ...
            <div className="floor-row" key={floor}>
              ...
              <div className="floor-apartments">
                {floorItems.length ? floorItems.map((apartment) => (
                  <button className={`drawer-apartment ${ownerClass(apartment.ownerContract || apartment.internalOwner || apartment.ownerName)} ${apartment.status === 'Leased' ? 'apartment-leased' : ''}`} key={apartment.id} onClick={() => onApartment(apartment)}>
...
function FinancialSummary({ rent, bills, events, transactions = [], transactionSearch = '', setTransactionSearch }) {
  ...
  const expectedTransactions = transactions.filter((transaction) => /^Rent expected$/i.test(String(transaction.Description || '').trim()));
  const recentTransactions = transactions.filter((transaction) => !/^Rent expected$/i.test(String(transaction.Description || '').trim()));
  const visibleTransactions = (transactionView === 'expected' ? expectedTransactions : recentTransactions).filter((transaction) => `${transaction.Description || ''} ${transaction.Vendor || ''} ${transaction['Order Date'] || ''} ${transaction.PurchaseId || ''} ${transaction.Amount || ''}`.toLowerCase().includes(transactionSearch.toLowerCase())).sort((a, b) => { const order = transactionDate(b['Order Date']) - transactionDate(a['Order Date']); return transactionSort === 'newest' ? order : -order; });
...
function ApartmentPhotoGallery({ apartment }) {
  const [index, setIndex] = useState(0);
  const [open, setOpen] = useState(false);
  const [remotePhotos, setRemotePhotos] = useState([]);
  const [photoCheckComplete, setPhotoCheckComplete] = useState(false);
  const localPhotos = apartment.photoUrls || [];
  const units = [...new Set(String(apartment.renterUnit || apartment.renterCsv?.unit || '').split('/').map((unit) => unit.trim()).filter(Boolean))];
  useEffect(() => { let cancelled = false; setPhotoCheckComplete(false); if (!units.length) { setRemotePhotos([]); setPhotoCheckComplete(true); return undefined; } Promise.all(units.map((unit) => fetch(`/api/photos?unit=${encodeURIComponent(unit)}`).then((response) => response.ok ? response.json() : { photos: [] }).catch(() => ({ photos: [] })))).then((results) => { if (!cancelled) { setRemotePhotos([...new Map(results.flatMap((result) => result.photos || []).map((photo) => [photo.key, photo])).values()]); setIndex(0); setPhotoCheckComplete(true); } }); return () => { cancelled = true; }; }, [apartment.id, units.join('|')]);
  const photos = [...remotePhotos, ...localPhotos];
  useEffect(() => { if (!open) return undefined; const closeOnKey = () => setOpen(false); window.addEventListener('keydown', closeOnKey); return () => window.removeEventListener('keydown', closeOnKey); }, [open]);
  if (!photoCheckComplete || !photos.length) return null;
  const previous = () => setIndex((index - 1 + photos.length) % photos.length);
  const next = () => setIndex((index + 1) % photos.length);
  return <section className="apartment-gallery"><div className="gallery-heading"><h3>Apartment photos</h3><span>{photos.length} photos</span></div>{photos.length > 0 ? <><div className="gallery-stage"><img src={photos[index].url} alt={photos[index].name} onClick={() => setOpen(true)} /><button type="button" className="gallery-arrow previous" onClick={previous}>‹</button><button type="button" className="gallery-arrow next" onClick={next}>›</button></div><small>{index + 1} / {photos.length} · Click the photo to enlarge</small></> : <p className="muted">Upload a photos ZIP to show images from the matching יחידה folder.</p>}{open && <div className="photo-lightbox" onClick={() => setOpen(false)}><button type="button" className="photo-lightbox-close" onClick={() => setOpen(false)}>×</button><img src={photos[index].url} alt={photos[index].name} onClick={(event) => event.stopPropagation()} /><button type="button" className="gallery-arrow previous" onClick={(event) => { event.stopPropagation(); previous(); }}>‹</button><button type="button" className="gallery-arrow next" onClick={(event) => { event.stopPropagation(); next(); }}>›</button><div className="photo-lightbox-caption">{index + 1} / {photos.length} · {photos[index].name}</div></div>}</section>;
}
function ApartmentSidePanel({ apartment, buildings, people = [], data, onBuilding, onClose }) {
  ...
  return (
    <aside className={`side-drawer apartment-drawer ${isInvestedGroup ? 'invested-apartment' : 'non-invested-apartment'}`} {...swipeHandlers}>
      ...
      <ApartmentPhotoGallery apartment={apartment} />
