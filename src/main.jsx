...
function App() {
  ...
  useEffect(() => { let cancelled = false; const units = [...new Set(data.apartments.flatMap((apartment) => String(apartment.renterUnit || apartment.renterCsv?.unit || '').split('/').map((unit) => unit.trim()).filter(Boolean)))]; if (!units.length) { setPhotoAvailability({}); return undefined; } (async () => { const available = {}; for (let index = 0; index < units.length && !cancelled; index += 1) { const unit = units[index]; try { const response = await fetch(`/api/photos?unit=${encodeURIComponent(unit)}`); const result = response.ok ? await response.json() : { photos: [] }; available[unitKey(unit)] = Boolean((result.photos || []).length); } catch { available[unitKey(unit)] = false; } if (index % 8 === 0 || index === units.length - 1) setPhotoAvailability({ ...available }); await new Promise((resolve) => setTimeout(resolve, 25)); } })(); return () => { cancelled = true; }; }, [data.apartments]);
