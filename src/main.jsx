...
const parseCoordinateCsv = (text) => { const lines = String(text || '').trim().split(/\r?\n/).filter(Boolean); if (lines.length < 2) return []; const headers = lines[0].split(',').map((header) => header.trim()); return lines.slice(1).map((line) => { const values = line.split(','); const row = Object.fromEntries(headers.map((header, index) => [header, String(values[index] || '').trim()])); return { ...row, lat: Number(row.lat), lng: Number(row.lng) }; }).filter((row) => Number.isFinite(row.lat) && Number.isFinite(row.lng)); };
const applyCoordinateCache = (buildings, coordinateRows) => buildings.map((building) => { const match = coordinateRows.find((row) => String(row.street || '').trim() === String(building.street || '').trim() && unitKey(row.streetNumber) === unitKey(building.streetNumber) && String(row.entrance || 'main').trim() === String(building.entrance || 'main').trim()) || coordinateRows.find((row) => String(row.name || '').trim() === String(building.name || '').trim()); return match ? { ...building, lat: match.lat, lng: match.lng, geoSource: 'coordinate-cache', geocodeAddress: addressKey({ ...building, entrance: building.entrance || match.entrance }) } : building; });
