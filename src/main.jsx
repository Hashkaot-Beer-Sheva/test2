...
const createUnifiedCanonicalData = (text, base = {}) => { const rows = parseUnifiedCsv(text); const buildings = []; const apartments = []; const buildingMap = new Map(); rows.forEach((row, index) => { const unifiedId = String(row[Object.keys(row)[0]] || '').trim(); if (!unifiedId || apartments.some((item) => unitKey(item.unifiedId) === unitKey(unifiedId))) return; const address = String(row['NEW OCCUPANCY ADRESSS'] || row['zehavit address'] || '').trim(); const match = address.match(/^(.+?)\s+(\d+[A-Za-zא-ת\/-]*)$/); const street = match?.[1] || address; const streetNumber = match?.[2] || ''; const key = `${normalizeCoordinateText(street)}|${unitKey(streetNumber)}`; let building = buildingMap.get(key); if (!building) { building = { id: `unified-building-${buildings.length + 1}`, name: `${street} ${streetNumber}`.trim() || `Unified building ${index + 1}`, street, streetNumber, address, area: 'Shechuna Gimel', units: 0, floors: 0, zone: 'Residential', color: 'gold', lat: null, lng: null }; buildingMap.set(key, building); buildings.push(building); } building.units += 1; apartments.push({ id: `unified-apartment-${unifiedId}`, unifiedId, buildingId: building.id, uniqueId: row['EASYPROD - UniquID'] || '', apptId: row['EASYPROD - Appt ID'] || '', number: '', entrance: '', street, streetNumber, addressNotEditable: address, altAddress: row['zehavit address'] || '', unifiedOccupancyUnit: row['NEW OCCUPANCY UNIT'] || '', unifiedOccupancyAddress: row['NEW OCCUPANCY ADRESSS'] || '', unifiedZehavitUnit: row['ZEHAVIT yechida'] || '', unifiedZehavitAddress: row['zehavit address'] || '', status: 'Vacant', rent: 0, tenantName: '', resident: '' }); }); return { ...base, buildings, apartments }; };
const enrichUnifiedWithMaster = (current, text) => { const imported = parseMasterCsv(text); const byKey = new Map((imported.apartments || []).map((item) => [`${unitKey(item.uniqueId)}|${unitKey(item.apptId)}`, item])); const byUnique = new Map((imported.apartments || []).map((item) => [unitKey(item.uniqueId), item])); const buildings = [...(current.buildings || [])]; const byAddress = new Map(buildings.map((item) => [`${normalizeCoordinateText(item.street)}|${unitKey(item.streetNumber)}`, item])); const apartments = (current.apartments || []).map((apartment) => { const source = byKey.get(`${unitKey(apartment.uniqueId)}|${unitKey(apartment.apptId)}`) || byUnique.get(unitKey(apartment.uniqueId)); if (!source) return apartment; const key = `${normalizeCoordinateText(source.street)}|${unitKey(source.streetNumber)}`; let building = byAddress.get(key); if (!building) { building = { id: `master-building-${buildings.length + 1}`, name: `${source.street} ${source.streetNumber}`.trim(), street: source.street, streetNumber: source.streetNumber, address: source.addressNotEditable || `${source.street} ${source.streetNumber}`, area: 'Shechuna Gimel', units: 0, floors: Number(source.floor || 0), zone: source.propertyClass || 'Residential', color: 'gold', lat: null, lng: null }; byAddress.set(key, building); buildings.push(building); } building.units = Number(building.units || 0) + 1; return { ...apartment, ...source, id: apartment.id, unifiedId: apartment.unifiedId, buildingId: building.id, unifiedOccupancyUnit: apartment.unifiedOccupancyUnit, unifiedOccupancyAddress: apartment.unifiedOccupancyAddress, unifiedZehavitUnit: apartment.unifiedZehavitUnit, unifiedZehavitAddress: apartment.unifiedZehavitAddress }; }); return { ...current, buildings, apartments }; };
...
function App() {
  ...
  const loadAllCloudData = () => { cloudDataLoaded.current = false; occupancyLoaded.current = false; setCloudLoadRequested(false); setTimeout(() => setCloudLoadRequested(true), 0); };
      ...
      <main>
        <header>
          ...
          <div className="actions top-search-wrap">
            ...
            <button className="outline small" onClick={loadAllCloudData} disabled={cloudDataLoading}>{cloudDataLoading ? 'Loading cloud data…' : 'Load all cloud data'}</button>
