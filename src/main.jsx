...
function App() {
  ...
  const coordinateLogSignature = React.useRef('');
  ...
  useEffect(() => { const signature = data.buildings.map((building) => `${building.id}:${building.lat}:${building.lng}`).join('|'); if (data.buildings.length && signature !== coordinateLogSignature.current) { coordinateLogSignature.current = signature; console.table(data.buildings.map((building) => ({ name: building.name, address: building.address || `${building.street || ''} ${building.streetNumber || ''}`, entrance: building.entrance || 'main', latitude: building.lat, longitude: building.lng, source: building.geoSource || 'not recorded' }))); } }, [data.buildings]);
  ...
  useEffect(() => { if (!authenticated) return undefined; let cancelled = false; fetch('/api/data').then((response) => response.ok ? response.json() : null).then((payload) => { if (!cancelled && payload?.files?.coordinates) setData((current) => ({ ...current, buildings: applyCoordinateCache(current.buildings, parseCoordinateCsv(payload.files.coordinates)) })); }).catch(() => {}); return () => { cancelled = true; }; }, [authenticated]);
  ...
  useEffect(() => { let cancelled = false; const units = [...new Set(data.apartments.flatMap((apartment) => String(apartment.renterUnit || apartment.renterCsv?.unit || '').split('/').map((unit) => unit.trim()).filter(Boolean)))]; if (!units.length) { setPhotoAvailability({}); return undefined; } Promise.all(units.map((unit) => fetch(`/api/photos?unit=${encodeURIComponent(unit)}`).then((response) => response.ok ? response.json() : { photos: [] }).catch(() => ({ photos: [] })).then((result) => [unitKey(unit), Boolean((result.photos || []).length)]))).then((entries) => { if (!cancelled) setPhotoAvailability(Object.fromEntries(entries)); }); return () => { cancelled = true; }; }, [data.apartments]);
